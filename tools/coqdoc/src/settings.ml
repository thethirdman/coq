...
(* Handles input and output files for coqdoc, including channels and
 * filetypes *)
type file_io = {
  i_file : string       ref;
...
(* Obtains the extension of a file from a path. Returns None if there is no
 * extension in the path, Some "ext" otherwise
 *)
let get_ext str =
...
(* Generates a file exntesion from a backend_type *)
let gen_ext prefix = function
  Html ->          (prefix ^ ".html")
  | Latex ->       (prefix ^ ".tex")
  | PrettyPrint -> (prefix ^ ".pp")

(* Takes a file extension and returns the associated frontend_type *)
